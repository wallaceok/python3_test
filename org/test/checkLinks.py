...
def getURL(url,session=None):
    ...
    if session is None:
        ...
        headers = {'contentType':'text/html;charset=UTF-8',
                   ...
                   'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/44.0.2403.4 Safari/537.36'}
    http = httplib2.Http(proxy_info = httplib2.ProxyInfo(socks.PROXY_TYPE_HTTP, "127.0.0.1", 8888))
    try:
...
def getSession(url, postData):
    header= {'Content-type': 'application/x-www-form-urlencoded','for-test':'1'}
    http = httplib2.Http(proxy_info = httplib2.ProxyInfo(socks.PROXY_TYPE_HTTP,"127.0.0.1", 8888),cache='.cache')
    response, content = http.request('http://app-member.fx.com/login?width=1920&height=1080','POST',urlencode(postData),headers=header,redirections=5)
    print(content.decode('utf-8'))
    headers = {'Cookie': response['set-cookie']}
    response, content = http.request('http://app-member.fx.com/', 'GET', headers=headers)
    print(content.decode('utf-8'))
    if response.status == 302:
        # headers['Cookie'] =  response['set-cookie']
        Location = response['location']
        response, content = http.request(Location, 'GET',urlencode(postData),headers=header)
        headers = {'Cookie': response['set-cookie']}
        if response.status == 200:
...
def main():
    homePage = 'http://app-member.fx.com' #首页链接
    ...
    if ifLogin:
        loginUrl = homePage
        postData = {'mobile':'13012345670','password':'123456'}
